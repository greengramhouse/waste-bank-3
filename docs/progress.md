...
> **อ่านตรงนี้ก่อนถ้าเพิ่งกลับมาทำต่อ**
>
> ระบบขึ้น GitHub Pages แล้วและใช้งานได้จริง — https://greengramhouse.github.io/waste-bank-3/app.html
> ฐานข้อมูลสะอาด (ธุรกรรม 0 · ยอดทุกคนศูนย์) พร้อมเปิดให้ครูใช้
>
> **ค้างไว้ 3 อย่าง**
> 1. โควต้า Firestore เต็มเมื่อ 4 ก.ย. จากการทดสอบทั้งวัน — สคริปต์ใน `tools/` รันไม่ได้จนกว่าจะรีเซ็ต
>    (เที่ยงคืน Pacific ≈ 14:00 น. เวลาไทย) **หน้าเว็บไม่กระทบ ครูใช้ได้ปกติ**
>    พอโควต้ากลับมา ให้รัน `npm run verify` และ `npm run balances -- --check` ยืนยันฐานอีกรอบ
> 2. จำกัด Browser API key ด้วย HTTP referrer — ยังไม่ได้ทำ ประโยชน์จริงคือกันเว็บปลอมที่ก๊อปหน้าเราไป
>    ใช้ key เดียวกันหลอกครูกรอกรหัส ส่วนที่เคยเขียนว่า "กันคนยิงจนโควต้าหมด" นั้นเกินจริง
>    เพราะปลอม Referer header ด้วย curl ได้ กำแพงจริงที่กันข้อมูลคือ Security Rules
> 3. แจกใบแจ้งรหัสและคู่มือให้ครู แล้วให้ทดลองใช้คู่ขนาน 1–2 สัปดาห์ก่อนตัดระบบ
...
## ทำเพิ่มเมื่อ 4 ก.ย. (รอบบ่าย)

- **ช่องค้นหานักเรียนในหน้าสรุปรายคน** แทน dropdown ที่ต้องเลื่อนหา — ค้นได้ทั้งชื่อ รหัส ห้อง
  (CLAUDE.md 8.2.5)
- **เมนูล้างธุรกรรมในหน้าตั้งค่า** สำหรับแอดมินที่รันสคริปต์ในเครื่องไม่ได้ ยืนยันสองชั้น
  ชั้นที่สองต้องพิมพ์คำว่า "ล้างข้อมูล" (CLAUDE.md 8.2.6)
- **แก้แถบ "มีรายการใหม่" ค้างหลังกดบันทึก** — ย้ายการเคลียร์สถานะไปไว้ใน `renderAllViews()` ที่เดียว
- **แยกระบบใหม่ออกจากระบบเดิมให้ครูมองเห็น** — ป้าย "ระบบใหม่ · ใช้รหัสผ่านที่ขึ้นต้นด้วย eco-"
  บนหน้าล็อกอิน ชื่อแท็บต่างกัน และข้อความตอนล็อกอินไม่ผ่านบอกให้ตรวจว่าที่อยู่ลงท้าย `/app.html`
  (ครูเคยเข้าหน้าแรกซึ่งยังเป็นระบบเดิมแล้วรหัสใหม่ใช้ไม่ได้)
